(function () {
  ...
  function bootDeratWidget() {
    ...
    shadow.innerHTML = `
      <style>
        ...
        .dt-close:focus-visible {
          outline: 3px solid rgba(41,115,115,.3);
          outline-offset: 3px;
        }
        ...
        @media (prefers-reduced-motion: reduce) {
          #derat-frame, #derat-teaser { transition: none !important; }
        }
      </style>
    ...
    function showTeaserLater(delay) {
      if (!widgetReady || dismissed || frame.classList.contains('derat-open')) return;
    ...
    function setOpen(open) {
      ...
      if (open) hideTeaser();
      else if (!dismissed) showTeaserLater(600);
    }

    function postToWidget(type) {
      try {
        frame.contentWindow.postMessage({ source: 'derat-parent', type: type }, WIDGET_ORIGIN);
      } catch (error) {}
    }
  ...
  if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', bootDeratWidget, { once: true });
  } else {
    bootDeratWidget();
  }
})();