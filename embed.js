(function(){
  ...
  function bootDeratWidget(){
    ...
    host.style.setProperty('overscroll-behavior','none','important');
    ...
    shadow.innerHTML=`
      <style>
        ...
        #derat-frame.derat-open{right:12px;bottom:10px;width:min(470px,calc(100vw - 24px));height:calc(100% - 20px);max-height:calc(100% - 20px)}
        ...
        #derat-mobile-bubble{all:unset;display:none;position:absolute;right:6px;bottom:4px;width:96px;height:96px;margin:0;padding:0;border:0;border-radius:50%;cursor:pointer;pointer-events:none;opacity:0;visibility:hidden;z-index:4;transform:scale(.94);transition:opacity .18s ease,transform .22s cubic-bezier(.2,.8,.2,1),visibility .18s ease;-webkit-tap-highlight-color:transparent;touch-action:manipulation}
        #derat-mobile-bubble.derat-show{opacity:1;visibility:visible;pointer-events:auto;transform:scale(1)}
        #derat-mobile-bubble .dmb-core{position:absolute;left:8px;top:8px;width:80px;height:80px;border-radius:50%;background:#ff7e47;display:flex;align-items:center;justify-content:center;box-shadow:0 0 0 5px rgba(255,255,255,.90),0 0 20px 5px rgba(255,126,71,.22);transition:transform .2s ease,background .2s ease,box-shadow .2s ease}
        #derat-mobile-bubble:active .dmb-core{transform:scale(.96);background:#f56f39}
        #derat-mobile-bubble img{display:block;width:48px;height:auto;user-select:none;pointer-events:none}
    ...
    var pageLocked=false,savedScrollX=0,savedScrollY=0,savedHtmlStyles=null,savedBodyStyles=null;

    function isMobile(){return mobileMedia.matches}
    function saveInlineStyles(element,names){var values={};names.forEach(function(name){values[name]=element.style.getPropertyValue(name)});return values}
    function restoreInlineStyles(element,values){Object.keys(values||{}).forEach(function(name){if(values[name])element.style.setProperty(name,values[name]);else element.style.removeProperty(name)})}
    ...
    function lockParentPage(){
      ...
      savedHtmlStyles=saveInlineStyles(document.documentElement,['overflow','overscroll-behavior']);
      savedBodyStyles=saveInlineStyles(document.body,['position','top','left','right','width','overflow','overscroll-behavior']);
      document.documentElement.style.setProperty('overflow','hidden');
      document.documentElement.style.setProperty('overscroll-behavior','none');
      document.body.style.setProperty('position','fixed');
      document.body.style.setProperty('top',(-savedScrollY)+'px');
      document.body.style.setProperty('left',(-savedScrollX)+'px');
    ...
    function unlockParentPage(){
      ...
      pageLocked=false;
      restoreInlineStyles(document.documentElement,savedHtmlStyles);
      restoreInlineStyles(document.body,savedBodyStyles);
    ...
    function showTeaserLater(delay){if(isMobile()||!widgetReady||dismissed||frame.classList.contains('derat-open'))return;if(teaserTimer)clearTimeout(teaserTimer);teaserTimer=setTimeout(function(){teaserTimer=0;if(!isMobile()&&!dismissed&&!frame.classList.contains('derat-open')){teaser.classList.add('derat-show');teaser.setAttribute('aria-hidden','false')}},delay)}
    function syncMobileBubble(){var shouldShow=isMobile()&&widgetReady&&!frame.classList.contains('derat-open');mobileBubble.classList.toggle('derat-show',shouldShow);mobileBubble.setAttribute('aria-hidden',shouldShow?'false':'true');if(isMobile())hideTeaser()}
    function markReady(){if(widgetReady)return;widgetReady=true;clearTimeout(readyFallbackTimer);frame.classList.add('derat-ready');syncMobileBubble();if(!isMobile())showTeaserLater(1400)}
    ...
    function setOpen(open){
      ...
      if(open){
        ...
        unlockParentPage();
        resetHostViewport();
        if(!dismissed&&!isMobile()) showTeaserLater(600);
      }
      syncMobileBubble();
    }
    ...
    window.addEventListener('message',function(event){
      ...
      if(data.type==='ready'){markReady();if(pendingOpen)postToWidget('open-calc');return}
      if(data.open===true){pendingOpen=false;setOpen(true);return}
      if(data.type==='close'||data.open===false){pendingOpen=false;setOpen(false)}
    });

    function handleViewportChange(){syncMobileBubble();syncVisualViewport()}
    ...
    window.addEventListener('orientationchange',function(){setTimeout(handleViewportChange,80)});
    ...
    window.addEventListener('pagehide',unlockParentPage);
